...
class NetworkedNNTPTestsMixin:
    ...
    def test_unknown_command(self):
        with self.assertRaises(nntp.NNTPPermanentError) as cm:
    ...
    def check_article_resp(self, resp, article, art_num=None):
        self.assertIsInstance(article, nntp.ArticleInfo)
...
class NetworkedNNTP_SSLTests(NetworkedNNTPTests):
    ...
    NNTP_CLASS = getattr(nntp, "NNTP_SSL", None)
...
class NNTPv1v2TestsMixin:
    ...
    def test_stat(self):
        ...
        self.assertEqual(message_id, "<45223423@example.com>")
        with self.assertRaises(nntp.NNTPTemporaryError) as cm:
    ...
    def test_too_long_lines(self):
        ...
        self.assertRaises(
            nntp.NNTPDataError, self.server.newnews, "comp.lang.python", dt
        )
...
class MiscTests(unittest.TestCase):
    def test_decode_header(self):
        def gives(a, b):
            self.assertEqual(nntp.decode_header(a), b)
    ...
    def test_parse_overview_fmt(self):
        ...
        self.assertEqual(
            nntp._parse_overview_fmt(lines),
            ["subject", "from", "date", "message-id", "references", ":bytes", ":lines"],
        )
    ...
    def test_parse_overview(self):
        fmt = nntp._DEFAULT_OVERVIEW_FMT + ["xref"]
        # First example from RFC 3977
        lines = [
        ...
        overview = nntp._parse_overview(lines, fmt)
    ...
    def test_parse_datetime(self):
        def gives(a, b, *c):
            self.assertEqual(nntp._parse_datetime(a, b), datetime.datetime(*c))
    ...
    def test_unparse_datetime(self):
        ...
        def gives(y, M, d, h, m, s, date_str, time_str):
            ...
            self.assertEqual(nntp._unparse_datetime(dt), (date_str, time_str))
            self.assertEqual(nntp._unparse_datetime(dt, False), (date_str, time_str))
    ...
    def test_unparse_datetime_legacy(self):
        ...
        def gives(y, M, d, h, m, s, date_str, time_str):
            dt = datetime.datetime(y, M, d, h, m, s)
            self.assertEqual(nntp._unparse_datetime(dt, True), (date_str, time_str))
    ...
    def test_ssl_support(self):
        self.assertTrue(hasattr(nntp, "NNTP_SSL"))
...
class PublicAPITests(unittest.TestCase):
    ...
    def test_module_all_attribute(self):
        self.assertTrue(hasattr(nntp, "__all__"))
        target_api = [
            "NNTP",
            "NNTPError",
            "NNTPReplyError",
        ...
        self.assertEqual(set(nntp.__all__), set(target_api))
...
class MockSocketTests(unittest.TestCase):
    ...
    def test_bad_welcome(self):
        ...
        self.check_constructor_error_conditions(
            Handler, nntp.NNTPProtocolError, Handler.welcome
        )
    ...
    def test_service_temporarily_unavailable(self):
        ...
        self.check_constructor_error_conditions(
            Handler, nntp.NNTPTemporaryError, Handler.welcome
        )
    ...
    def test_service_permanently_unavailable(self):
        ...
        self.check_constructor_error_conditions(
            Handler, nntp.NNTPPermanentError, Handler.welcome
        )
    ...
    def test_bad_capabilities(self):
        ...
        self.check_constructor_error_conditions(
            Handler, nntp.NNTPReplyError, capabilities_response
        )
    ...
    def test_login_aborted(self):
        ...
        self.check_constructor_error_conditions(
            Handler, nntp.NNTPPermanentError, authinfo_response, login, password
        )
...
class MockSslTests(MockSocketTests):
    ...
    def nntp_class(*pos, **kw):
        return nntp.NNTP_SSL(*pos, ssl_context=bypass_context, **kw)
